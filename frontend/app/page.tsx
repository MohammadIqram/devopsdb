  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Console Status Overview</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Active environment pipelines and open critical incidents for {selectedRepo || 'no repository selected'}
          </p>
        </div>
        <button
          onClick={fetchData}
          disabled={loading || !selectedRepo}
          className={`px-4 py-2.5 rounded-lg border text-sm font-medium transition flex items-center gap-2 cursor-pointer ${theme === 'dark'
